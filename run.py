...
def test_db():
    try:
        # use text(...) for textual SQL expressions
        ...
        # keep logs for debugging and clean up session so future requests don't fail
        current_app.logger.exception("test-db failed")
        try:
            db.session.rollback()
        except Exception:
            pass
...
if __name__ == "__main__":
    host = os.getenv("FLASK_RUN_HOST", "0.0.0.0")
    port = int(os.getenv("FLASK_RUN_PORT", 5000))
    # Force debug mode off for production safety
    debug = False
