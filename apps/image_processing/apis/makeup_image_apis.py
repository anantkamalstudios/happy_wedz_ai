        ...
        products_list = [
            {
                ...
                "product_real_image": f"data:{p.product_real_image_type};base64," +
                                      base64.b64encode(p.product_real_image).decode("utf-8"),
                
            }
            for p in dc.products  # dc.products is the backref relationship
        ]

        result.append({
            "product_detailed_category_name": dc.name,
            "product_detailed_image": f"data:{dc.image_type};base64," + base64.b64encode(dc.image).decode("utf-8") if dc.image else None,
